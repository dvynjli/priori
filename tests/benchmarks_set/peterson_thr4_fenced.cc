...
void* t3(void *){
	int rwaiting0=-1, rwaiting1=-1, rwaiting2=-1, rlevel0=-1, rlevel1=-1, rlevel2=-1, rx=-1;
	level3.store(0,REL);
	//__fence_var.fetch_add(0, ACQREL);
	...
	rlevel1 = level1.load(ACQ);
	rlevel2 = level2.load(ACQ);
	assume(rwaiting1!=3 || (rlevel0 < 1 && rlevel1 < 1 && rlevel2 < 1 && 1));
	level3.store(2,REL);
	//__fence_var.fetch_add(0, ACQREL);
	...
	rlevel1 = level1.load(ACQ);
	rlevel2 = level2.load(ACQ);
	assume(rwaiting2!=3 || (rlevel0 < 2 && rlevel1 < 2 && rlevel2 < 2 && 1));
	_cc_x.store(3,REL);
	rx = _cc_x.load(ACQ);
	assert(rx==3);
	//__fence_var.fetch_add(0, ACQREL);
