...
void *t0(void *arg) {
	...
	tdiff0.store(1, memory_order_release);
	__fence_var.fetch_add(0, memory_order_acq_rel);
	rchildnotready = childnotready.load(memory_order_acquire);
	assume(rchildnotready==0);
	childnotready.store(1, memory_order_release);
	parentsense.store(rsense, memory_order_release);
	rsense = 1 - rsense;
	__fence_var.fetch_add(0, memory_order_acq_rel);
	rtdiff1 = tdiff1.load(memory_order_acquire);
	assert(rtdiff1!=0);
	return NULL;
}
...
void *t1(void *arg) {
	...
	childnotready.store(0, memory_order_release);
	__fence_var.fetch_add(0, memory_order_acq_rel);
	rparentsense = parentsense.load(memory_order_acquire);
	assume(rparentsense==rsense);
	rsense = 1 - rsense;
	__fence_var.fetch_add(0, memory_order_acq_rel);
	rtdiff0 = tdiff0.load(memory_order_acquire);
	assert(rtdiff0!=0);
	return NULL;
}
